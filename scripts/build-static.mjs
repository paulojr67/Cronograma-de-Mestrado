// Build estático para Netlify / Vercel / GitHub Pages
// Gera site 100% estático lendo dados via API do servidor PM2 local
...
console.log('🏗️  Build estático para Netlify')
...
const { tasks, phases, weeks, criticalPath, stats, partners } = fullData
console.log(`   ✓ ${tasks.length} tarefas, ${partners.length} parceiros`)

// ============ LIMPAR E CRIAR DIRETÓRIO ============
await fs.rm(OUT_DIR, { recursive: true, force: true })
await fs.mkdir(OUT_DIR, { recursive: true })
await fs.mkdir(path.join(OUT_DIR, 'static'), { recursive: true })
...
await copyDir(path.join(PUBLIC_DIR, 'static'), path.join(OUT_DIR, 'static'))
console.log('   ✓ Assets copiados (CSS + JS)')

// ============ GERAR API ESTÁTICA (JSON) ============
await fs.writeFile(path.join(OUT_DIR, 'api', 'schedule'), JSON.stringify(fullData, null, 2))

await fs.mkdir(path.join(OUT_DIR, 'api', 'tasks'), { recursive: true })
for (const phase of phases) {
  const phaseTasks = tasks.filter(t => t.phaseId === phase.id)
  await fs.writeFile(
    path.join(OUT_DIR, 'api', 'tasks', phase.id),
    JSON.stringify({ phaseId: phase.id, count: phaseTasks.length, tasks: phaseTasks }, null, 2)
  )
}

const criticalTasks = tasks.filter(t => criticalPath.includes(t.id))
await fs.writeFile(
  path.join(OUT_DIR, 'api', 'critical-path'),
  JSON.stringify({ count: criticalTasks.length, tasks: criticalTasks }, null, 2)
)

await fs.mkdir(path.join(OUT_DIR, 'api', 'partners'), { recursive: true })
for (const partner of partners) {
  const partnerTasks = tasks.filter(t => t.partnerId === partner.id)
  await fs.writeFile(
    path.join(OUT_DIR, 'api', 'partners', partner.id),
    JSON.stringify({ partner, count: partnerTasks.length, tasks: partnerTasks }, null, 2)
  )
}
console.log('   ✓ Endpoints API estáticos gerados')
  ...
  <main class="max-w-[1600px] mx-auto px-4 md:px-6 py-6 md:py-8">
    ...
    <section id="tab-critical" class="tab-content hidden">
      <div class="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 rounded-lg p-5 mb-6">
        ...
        <p class="text-red-800 text-sm">Estas são as tarefas que <strong>não podem atrasar de jeito nenhum</strong>. Qualquer atraso aqui empurra a data da Qualificação. O sequenciamento abaixo garante que a <strong>prova de conceito da nanoformulação</strong> e os <strong>primeiros resultados de inibição enzimática + Zebrafish</strong> estejam prontos para Agosto/2026.</p>
      </div>
    ...
    <section id="tab-writing" class="tab-content hidden">
      <div class="bg-gradient-to-r from-slate-50 to-slate-100 border-l-4 border-slate-500 rounded-lg p-5 mb-6">
        ...
        <p class="text-slate-700 text-sm"><strong>Princípio chave:</strong> escrever EM TEMPO REAL, não no final. Cada semana de bancada deve gerar 1-2 páginas de Materiais & Métodos. A revisão bibliográfica começa na <strong>Semana 1</strong> e segue paralela ao trabalho experimental.</p>
      </div>
...
console.log('   ✓ netlify.toml + _headers')
...
console.log('\n✅ Build concluído em ' + OUT_DIR)
console.log('\n📦 Para deploy:')
console.log('   1. Drag & drop em https://app.netlify.com/drop')
console.log('   2. Ou compacte e suba via Git\n')
